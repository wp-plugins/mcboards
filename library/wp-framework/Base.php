...
abstract class WpFramework_Base_0_6 extends WP_Widget {
    ...
    public function getNavTabs($tabs, $current=null) {
        $html = "";
        $html_tab_format = '<a class="nav-tab%s" id="nav-tab-%s" href="%s">%s</a>';
        foreach($tabs as $key => $name) {
            ...
            $html .= sprintf($html_tab_format, $class, $key, $this->get_current_url(array('tab','board_id', 'updating')) . '&tab=' . $key, $name);
        }
        return $html;
    }
